...
async function deleteJob() {
  ...
  try {
    const job = await Job.fromId(queue, JOB_ID_TO_DELETE);
    ...
    await job.remove(); // Remove the job completely
    console.log(`Job ${JOB_ID_TO_DELETE} successfully removed.`);

    // Verify removal
    const jobAfterRemove = await Job.fromId(queue, JOB_ID_TO_DELETE);
    if (!jobAfterRemove) {
